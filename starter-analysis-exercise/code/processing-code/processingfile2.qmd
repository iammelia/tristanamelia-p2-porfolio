...
#Leo did not clean the data, I added the following code myself.

d5 <- d4 %>%dplyr::filter(`Hair Color` %in% c("blond", "brown", "black", "white", "other"))
skimr::skim(d5)

d6 <- d5 %>% dplyr::filter(Inseam > 2)
skimr::skim(d5)
```
...
```{r}
#leo did not fix the processed data copy
